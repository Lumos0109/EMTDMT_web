...
class Mymodel_NoAux(nn.Module):
    def __init__(self,
        ...
        super(Mymodel_NoAux, self).__init__()
    ...
    def evaluate(self, loader):
        ...
        self.eval()
        with torch.no_grad():
            for ch_datas, pls_datas, pais_datas, labels in loader:
                ch_datas, pls_datas, pais_datas, labels = ch_datas.to(
                    device), pls_datas.to(device), pais_datas.to(device), labels.to(
                    device)
                prediction = self(ch_datas, pls_datas, pais_datas)
                _, prediction = torch.max(prediction, 1)
                correct += (prediction == labels).sum()
                total += labels.size(0)
                for class_id in range(self.class_number):
...
class CNN_LSTM(nn.Module):
    def __init__(self):
        ...
        self.softmax = nn.Sequential(
            nn.Linear(256 * 3, self.class_number),
            nn.Softmax(dim=1)
        )

    def forward(self, pkt1, pkt2, pkt3):
        h_state = None
        pkt1 = self.fc(self.conv(pkt1.float().reshape(-1, 1, 28, 28)).reshape(-1, 6272))
        pkt2 = self.fc(self.conv(pkt2.float().reshape(-1, 1, 28, 28)).reshape(-1, 6272))
        pkt3 = self.fc(self.conv(pkt3.float().reshape(-1, 1, 28, 28)).reshape(-1, 6272))
        pkts = torch.cat([pkt1.unsqueeze(1), pkt2.unsqueeze(1), pkt3.unsqueeze(1)], dim=1)
        out, _ = self.lstm(pkts, h_state)
        out = self.softmax(out.reshape(-1, 3 * 256))
        return out
    ...
    def evaluate(self, loader):
        ...
        self.eval()
        with torch.no_grad():
            for pkt1, pkt2, pkt3, labels in loader:
                pkt1, pkt2, pkt3, labels = pkt1.to(
                    device), pkt2.to(device), pkt3.to(device), labels.to(
                    device)
                prediction = self(pkt1, pkt2, pkt3)
                _, prediction = torch.max(prediction, 1)
                correct += (prediction == labels).sum()
                total += labels.size(0)
                for class_id in range(self.class_number):
